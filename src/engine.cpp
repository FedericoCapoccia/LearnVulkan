...
void Engine::init_vulkan()
{
    ...
    const auto& [graphics_queue, swapchain, device] = m_GpuManager.init(spec);
...
bool Engine::draw_frame()
{
    ...
    vk::Result result = m_Device.acquireNextImageKHR(
        m_SwapchainBundle.Handle, UINT64_MAX, get_current_frame().SwapChainSemaphore, VK_NULL_HANDLE, &image_index);
